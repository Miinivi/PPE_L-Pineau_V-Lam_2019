 ...
 */
  ...
  if ($etape=='validerConnexion') { // un client demande à s'authentifier
      ...
      if ( is_array($lgUser[0]) ) { 
          affecterInfosConnecte($lgUser[0]["id"], $lgUser[0]["login"]);
      }
      else {
    ...
    if ( $etape == "validerConnexion" && nbErreurs($tabErreurs) == 0 ) {
        if($lgUser[1] == 'visiteur'){
            header("Location:cAccueil.php");
        }
        if($lgUser[1] == 'comptable'){
...
?>               
      ...
      <div class="piedForm">
      <p>
        <input type="submit" id="ok" value="Valider" class="button" />
        <input type="reset" id="annuler" value="Effacer" class="button"/>
      </p> 
      </div>
      </form>
    </div>
